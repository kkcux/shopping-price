...
const matchProduct = (targetName, candidates) => {
  ...
  for (const c of candidates) {
    ...
    const overlap = baseTokens.filter((t) => tokens.includes(t)).length;
...
export default function MyLists3() {
  ...
  const lotusList = useMemo(() => (Array.isArray(lotusData) ? lotusData : []), []);
  const makroList = useMemo(() => (Array.isArray(makroData) ? makroData : []), []);
  ...
  const items = useMemo(() => {
    ...
    return base.map((l) => {
      ...
      const validPrices = Object.values(priceMap).filter((v) => typeof v === "number");
  ...
  const totals = useMemo(() => {
    const sum = (key) => items.reduce((acc, it) => acc + (toNumber(it.priceMap[key]) || 0), 0);
    return {
      nonMember: {
        LOTUS: sum("LOTUS"),
        BIGC: null,
  ...
  /* ===== ร้านแนะนำ ===== */
  const shops = useMemo(() => [
    { key: "BIGC", brand: "BIG C", distance: "2.5 km", member: null, nonMember: null, actionText: "ไปยังร้านค้า" },
    { key: "LOTUS", brand: "LOTUS's", distance: "2.8 km", member: totals.member.LOTUS, nonMember: totals.nonMember.LOTUS, actionText: "ไปยังร้านค้า" },
    { key: "MAKRO", brand: "MAKRO", distance: "3.7 km", member: totals.member.MAKRO, nonMember: totals.nonMember.MAKRO, actionText: "ไปยังร้านค้า" },
  ], [totals]);
          ...
          <section className="ml3-block">
            ...
            <div className="ml3-table">
              ...
              <div className="ml3-tr total">
                <div className="ml3-td left total-label">รวมราคาทั้งหมด ( ไม่เป็นสมาชิก )</div>
                {["LOTUS", "BIGC", "MAKRO"].map((k) => (
                  <div className="ml3-td" key={k}>
                    <span className="ml3-pill">
                      {typeof totals.nonMember[k] === "number" ? Math.round(totals.nonMember[k]) : "-"}
                    </span>
                  </div>
                ))}
              </div>
              <div className="ml3-tr total">
                <div className="ml3-td left total-label">รวมราคาทั้งหมด ( เป็นสมาชิก )</div>
                {["LOTUS", "BIGC", "MAKRO"].map((k) => (
                  <div className="ml3-td" key={k}>
                    <span className={`ml3-pill ${k === "LOTUS" ? "best" : ""}`}>
                      {typeof totals.member[k] === "number" ? Math.round(totals.member[k]) : "-"}
                    </span>
                  </div>
                ))}
              </div>
            ...
            <div className="ml3-shop-table">
              ...
              {shops.map((s) => (
                <div className="ml3-shop-row" key={s.key}>
                  ...
                  <div className="ml3-shop-price">{typeof s.member === "number" ? Math.round(s.member) : "-"}</div>
                  <div className="ml3-shop-price">{typeof s.nonMember === "number" ? Math.round(s.nonMember) : "-"}</div>
                  <div className="ml3-shop-action">
                    <button className="ml3-go">{s.actionText} ›</button>
                  </div>
                </div>
              ))}
            </div>
